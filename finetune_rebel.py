...
# Merge with implicit causality examples (147 examples generated by LLM)
try:
    from rebel_implicit_causality_training import (
        HEALTHCARE_IMPLICIT, FINANCE_IMPLICIT, MANUFACTURING_IMPLICIT
    )
    _hc_implicit = [RelationExample(text=e["text"], relations=e["relations"]) for e in HEALTHCARE_IMPLICIT]
    _fi_implicit = [RelationExample(text=e["text"], relations=e["relations"]) for e in FINANCE_IMPLICIT]
    _mfg_implicit = [RelationExample(text=e["text"], relations=e["relations"]) for e in MANUFACTURING_IMPLICIT]
except ImportError:
    _hc_implicit = _fi_implicit = _mfg_implicit = []

ALL_EXAMPLES = {
    "healthcare": HEALTHCARE_EXAMPLES + _hc_implicit,
    "finance": FINANCE_EXAMPLES + _fi_implicit,
    "manufacturing": _mfg_implicit,  # new domain
}
